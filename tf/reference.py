...
class CTCModel():
    ...
    def add_placeholders(self):
        ...
        self.seq_lens_placeholder = tf.placeholder(tf.int32)
    ...
    def add_prediction_op(self):
        ...
        b = tf.get_variable('b', shape=[Config.num_classes], initializer = tf.constant_initializer(0))

        cells = list()
        cells.append(tf.contrib.rnn.GRUCell(Config.num_hidden))
        cell = tf.contrib.rnn.MultiRNNCell(cells)
        #outputs, state = tf.nn.dynamic_rnn(cell, self.inputs_placeholder, time_major=True, dtype=tf.float32)
        outputs, state = tf.nn.dynamic_rnn(cell, self.inputs_placeholder, dtype=tf.float32)
        
        batch_size = tf.shape(outputs)[0]
        max_timestep = tf.shape(outputs)[1]

        outputs = tf.reshape(outputs, [-1, Config.num_hidden])
        logits = tf.matmul(outputs, W) + b
        logits = tf.reshape(logits, [batch_size, max_timestep, Config.num_classes])
        logits = tf.transpose(logits, (1, 0, 2))

        self.l2_cost = tf.nn.l2_loss(W)
    ...
    def add_loss_op(self):
        ...
        self.loss = Config.l2_lambda * self.l2_cost + cost               
    ...
    def add_decoder_and_wer_op(self):
        ...
        """        
        decoded_sequence, prop = tf.nn.ctc_beam_search_decoder(self.logits, self.seq_lens_placeholder)
        wer = tf.reduce_mean(tf.edit_distance(tf.cast(decoded_sequence[0], tf.int32), self.targets_placeholder))

        tf.summary.scalar("loss", self.loss)
        tf.summary.scalar("wer", wer)

        self.decoded_sequence = decoded_sequence[0]
        self.wer = wer
