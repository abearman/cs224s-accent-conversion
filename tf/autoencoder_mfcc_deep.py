...
def encode(current_input, max_num_frames, num_mfcc_coeffs, state_size_1, state_size_2, state_size_3, version):
    ...
    W3 = tf.get_variable("W3"+version, shape=(state_size_2, state_size_3), initializer=xavier) 
    ...
    b3 = tf.get_variable('b3'+version, shape=[state_size_3], initializer = tf.constant_initializer(0))
...
def decode(encoder, current_input, max_num_frames, num_mfcc_coeffs, state_size_1, state_size_2, state_size_3, version):
    encoder.reverse()
    W1 = tf.transpose(encoder[0])
    W2 = tf.transpose(encoder[1])
    W3 = tf.transpose(encoder[2])
    b1 = tf.get_variable('b4'+version, shape=[state_size_3], initializer = tf.constant_initializer(0))
    b2 = tf.get_variable('b5'+version, shape=[state_size_2], initializer = tf.constant_initializer(0))
    b2 = tf.get_variable('b6'+version, shape=[num_mfcc_coeffs], initializer = tf.constant_initializer(0))
...
def autoencoder(max_num_frames, num_mfcc_coeffs, state_size_1, state_size_2, state_size_3, version):
    ...
    encoder, output = encode(current_input, max_num_frames, num_mfcc_coeffs, state_size_1, state_size_2, state_size_3, version)
    
    decoded = decode(encoder, output, max_num_frames, num_mfcc_coeffs, state_size_1, state_size_2, state_size_3, version)
...
def autoencode():
    ...
    ae1 = autoencoder(max_num_frames, num_mfcc_coeffs, state_size_1, state_size_2, state_size_3, '1')
    ae2 = autoencoder(max_num_frames, num_mfcc_coeffs, state_size_1, state_size_2, state_size_3, '2')
    ...
    for i, batch in enumerate(minibatches):
        encode_W1 = encoder[2].eval(session=sess)
        encode_W2 = encoder[1].eval(session=sess)
        encode_W3 = encoder[0].eval(session=sess)
        decode_W1 = np.transpose(decoder[0].eval(session=sess))
        decode_W2 = np.transpose(decoder[1].eval(session=sess))
        decode_W3 = np.transpose(decoder[2].eval(session=sess))
        
        h1 = np.tanh(batch_multiply_by_matrix_numpy(np.array(batch), encode_W1)) + encoder[6].eval(session=sess)
        h2 = np.tanh(batch_multiply_by_matrix_numpy(h1, encode_W2)) + encoder[5].eval(session=sess)
        h3 = np.tanh(batch_multiply_by_matrix_numpy(h2, encode_W3)) + encoder[4].eval(session=sess)
        h4 = np.tanh(batch_multiply_by_matrix_numpy(h3, decode_W1)) + decoder[4].eval(session=sess)
        h5 = np.tanh(batch_multiply_by_matrix_numpy(h4, decode_W2)) + decoder[5].eval(session=sess)
        prediction = batch_multiply_by_matrix_numpy(h4, decode_W3) + decoder[6].eval(session=sess)
