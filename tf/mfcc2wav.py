...
def mfcc2wav(data, mfcc, samplerate=16000, numcep=13, ceplifter=22, appendEnergy=True):
		"""Converts a mfcc feature array to a wav file.
		Args:
			mfcc: numpy array of shape (num_frames, n_mfcc_features)
		"""
		#if appendEnergy:
		#	mfcc[:,0] = np.exp(energy)
		mfcc = ilifter(mfcc, ceplifter)
		log_spectrum = idct(mfcc, type=2, axis=1, norm='ortho') #[:,:numcep]	
		#spectrum = np.exp(mfcc)
		print "spectrum: ", log_spectrum.shape
		spectrum = stft(data)
		back_y = istft(spectrum)
		print "y: ", len(data)
		print "back_y: ", len(back_y)
		wavfile.write('test_target.wav', samplerate, back_y) 
...
def ilifter(cepstra, L=22):
	if L > 0:
				nframes, ncoeff = np.shape(cepstra)
				n = np.arange(ncoeff)
				lift = 1 + (L/2.0) * np.sin(np.pi * n / L)
				return (1.0/lift) * cepstra
	else:
